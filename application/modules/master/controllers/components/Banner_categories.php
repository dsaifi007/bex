...
class Banner_categories extends AdminMaster {

    protected $item_ID;
    protected $parent_dir = 'components';
    protected $list_link = 'components/banner_categories';
    protected $tbl = 'banner_categories';
    
    protected $model_name = 'components/banner_category_model';
    protected $model = 'banner_category_model';
    protected $is_model = 0;

    protected $utility_cnfg = [
        'select'=> ['a.id', 'a.category_name', 'b.title'],
        'filters'=> ['where'=>['a.published'=>1, 'b.published'=>1]],
        'hooks'=> ['FormattedResultList(category_name.title)']
    ];
    ...
    private function _initEnv() {
        $this->lang->load($this->parent_dir.'/banner_categories');
        $this->_loadModelEnv();
    }
    ...
    public function index() {
        ...
        if (count($data['items']) > 0) :
            ...
            $this->minify->add_js($this->th_custom_js_path.$this->parent_dir.'/banner_categories.js', 'page');
        endif;

        $data['list_link'] = $this->setAppURL($this->list_link);
        $data['view_name'] = $this->parent_dir.'/banner_categories';
        $data['view_class'] = 'banner_categories';
    ...
    private function _initBasicFormEnv($data) {
        ...
        if ($this->item_ID) :
            ...
            $lang = 'edit_banner_category';
            $data['add_link'] = $this->setAppURL($this->list_link.'/add');
        else :
            $data['form_action'] = $this->list_link . '/add';
            $lang = 'add_banner_category';
        endif;
        $this->lang->load($this->parent_dir.'/'.$lang);
        $data['form_action'] = $this->setAppURL($data['form_action']);
        $data['list_link'] = $this->setAppURL($this->list_link);
        //$data = $this->BuildAdminEnv($data);
        $this->minify->add_js($this->th_custom_js_path.$this->parent_dir.'/banner_category_form.js', 'page');
        $data['view_name'] = $this->parent_dir.'/banner_category_form';
        $data['view_class'] = 'banner_category_form_blk';
        return $data;
    }
    ...
    protected function doFormValidation() {
        ...
        $this->form_validation->set_rules('category_name', 'lang:err_banner_category_name', 'trim|required|min_length[2]|max_length[80]'.$is_unique);
        $this->form_validation->set_rules('domain_id', 'lang:err_banner_category_domain','trim|required|in_list['.implode(',',array_keys($this->domains_list)).']', ['in_list'=>$this->lang->line('err_in_list')]);
        $multi_unique_param = [
            'id'=>$this->item_ID,
            'fields'=> ['category_name', 'domain_id'],
            'table'=>$this->tbl
        ];
        $this->form_validation->set_rules('category_name', 'lang:err_banner_category_name', 'multi_is_unique['.json_encode($multi_unique_param).']');
    ...
    protected function FormProcess($data) {
        ...
        if ($this->doFormValidation()) :
            ...
            if ($item_id) :
                $data['success'] = $this->lang->line('success_banner_category_name_save');
            ...
            else : $data['error'] = $this->lang->line('err_banner_category_name_save');
            endif;
        endif;
    ...
    public function delete($id) {
        ...
        $this->session->set_flashdata('flashSuccess', $this->lang->line('success_banner_category_name_delete'));
        $this->deleteCache($this->cache_list);
