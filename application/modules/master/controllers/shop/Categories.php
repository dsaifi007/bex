...
class Categories extends AdminMaster {

    protected $item_ID;
    protected $parent_dir = 'shop';
    protected $list_link = 'shop/categories';
    protected $tbl = 'shop_categories';
    protected $model_name = 'shop/Category_model';
    ...
    private function _initEnv() {
        $this->loadLanguage($this->parent_dir . '/categories');
        $this->_loadModelEnv();
    ...
    public function index() {
        ...
        if (count($data['items']) > 0) :
            ...
            $this->minify->add_js($this->th_custom_js_path . $this->parent_dir . '/categories.js', 'page');
        endif;

        $data['list_link'] = $this->setAppURL($this->list_link);
        $data['view_name'] = $this->parent_dir . '/Categories';
        $data['view_class'] = 'shop_categories';
    ...
    private function _initBasicFormEnv($data) {
        ...
        $data['view_class'] = 'shop_category_form_blk';
        return $data;
    }
    ...
    protected function doFormValidation() {
        ...
        $this->form_validation->set_rules('stores[]', 'lang:err_category_stores','trim|required|in_list['.implode(',',array_keys($this->stores_list)).']', ['in_list'=>$this->lang->line('err_in_list')]);
    ...
    protected function FormProcess($data) {
        ...
        $this->item_ordering_list = $data['item_ordering_list'] = ($this->item_ID > 0) ? $this->get_Items(['filterArr' => ['db_Select' => ['a.id', 'a.item_ordering', 'a.category_name'], 'db_Joins' => [], 'db_Order' => ['a.item_ordering' => 'asc'], 'db_Filters' => ['where' => ['a.parent_id' => $this->{$this->model}->data_item->parent_id]]], 'hooks' => ['FormattedResultListOnIndex(item_ordering.category_name,. ,item_ordering)']]) : [];
    ...
    public function utilityProductsCategoriesList($noParentList=0) {
        $hooks = ['treeOrder'];
        if(!$noParentList) :
            $this->load->helper('string');
            array_push($hooks, 'getParentsList(category_name)');
        endif;
        return $this->get_Items(['filterArr'=>['db_Select'=>['a.id', 'a.parent_id', 'a.category_name'], 'db_Filters'=>['where'=>['a.published'=>1]]], 'hooks'=>$hooks]);
    }
