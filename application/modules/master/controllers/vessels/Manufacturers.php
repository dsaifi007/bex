...
class Manufacturers extends AdminMaster {
    ...
    protected $list_link = 'vessels/manufacturers';
    protected $tbl = 'vessels_manufacturers';
    
    protected $model_name = 'vessels/Manufacturer_model';
    protected $model = 'manufacturer_model';
    protected $is_model = 0;

    protected $utility_cnfg = [
        'select'=> ['a.id', 'a.manufacturer_name'],
        'filters'=> ['where'=>['a.published'=>1]],
        'hooks'=> ['FormattedResultList(manufacturer_name)']
    ];
    ...
    private function _initBasicFormEnv($data) {
        ...
        $data['view_class'] = 'manufacturer_blk';
        return $data;
    }

    protected function doFormValidation() {
        $this->BuildFormValidationEnv();
        $is_unique = ($this->item_ID > 0 && strtolower($this->input->post('manufacturer_name')) == strtolower($this->{$this->model}->data_item->manufacturer_name)) ? '' : '|is_unique[' . $this->tbl . '.manufacturer_name]';
        $this->form_validation->set_rules('manufacturer_name', 'lang:err_manufacturer_name', 'trim|required|min_length[2]|max_length[50]'.$is_unique);
    ...
    protected function FormProcess($data) {
        ...
        if ($this->doFormValidation()) :
            $item_id = $this->{$this->model}->save(arrange_post_data($this->input->post(), [ 'manufacturer_name'=> '','published' =>1]));
            if ($item_id) :
                $data['success'] = $this->lang->line('success_manufacturer_save');
            ...
            else : $data['error'] = $this->lang->line('err_manufacturer_save');
            endif;
        endif;
    ...
    public function delete($id) {
        ...
        $this->session->set_flashdata('flashSuccess', $this->lang->line('success_manufacturer_delete'));
        $this->deleteCache($this->tbl . '*');
