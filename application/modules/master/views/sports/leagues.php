...
$table_heads = [
    ['data' => $this->lang->line('tb_hd_league_name'), 'class' => 'left', 'width' => '30%'],
    ['data' => $this->lang->line('tb_hd_league_teams'), 'class' => 'left', 'width' => '20%'],
    ['data' => $this->lang->line('tb_hd_league_countries'), 'class' => 'left', 'width' => '10%'],
    ['data' => $this->lang->line('tb_hd_league_starts_on'), 'class' => 'text-center hidden-xs', 'width' => '10%'],
    ['data' => $this->lang->line('tb_hd_league_ends_on'), 'class' => 'text-center hidden-xs', 'width' => '10%'],
...
$template_active = ['table_open' => '<table class="table table-striped table-bordered table-hover leagues_list">'];
            ...
            <div class="portlet-body">
                ...
                if (count($items) > 0) :
                    ...
                    foreach ($items as $k => $v) :
                        ...
                        $teams = (is_array($v->teams))? $v->teams:[];
                        array_walk($teams, function(&$n, $k, $mixed) {
                            $n = (isset($mixed[$n]))? $mixed[$n]:'';
                        }, $teams_list);

                        $countries = (is_array($v->country_codes))? $v->country_codes:[];
                        array_walk($countries, function(&$n, $k, $mixed) {
                            $n = (isset($mixed[$n]))? $mixed[$n]:'';
                        }, $countries_list);

                        $rowdata = [];
                        $rowdata[] = ['data' => $v->league_name];
                        $rowdata[] = ['data' => display_label_list(array_filter($teams))];
                        $rowdata[] = ['data' => display_label_list(array_filter($countries))];
                        $rowdata[] = ['data' => $v->starts_on, 'class' => 'text-center hidden-xs'];
                        $rowdata[] = ['data' => $v->ends_on, 'class' => 'text-center hidden-xs'];
